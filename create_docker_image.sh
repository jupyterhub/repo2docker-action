...
if [ -d "${PWD}/image-tests" ]; then
    ...
    docker run -u 1000 -v "${PWD}/image-tests:/tmp/image-tests" -w ${REPO_DIR} \
        ...
        [ -f /tmp/image-tests/requirements.txt ] && \
            echo "Installing from image-tests/requirements.txt..." && \
            python3 -m pip install --no-cache -r /tmp/image-tests/requirements.txt;
        ...
        ls /tmp/image-tests/*.ipynb > /dev/null && \
        ...
        py.test ${PYTEST_FLAGS} /tmp/image-tests/
    '
    echo "::endgroup::"
fi
